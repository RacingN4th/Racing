...
EXP_ST u8 virgin_bits[MAP_SIZE], /* Regions yet untouched by fuzzing */
    ...
    virgin_bits_crashes[INST_SIZE*64], virgin_bits_non_crashes[INST_SIZE*64],
...
char inst_predicate[INST_SIZE];  // type of instruction predicate ('>' or '<')
...
u32 new_counterexample_for_predicate_cnt = 0;    // counter for predicate with counter examples
double counterexample_rate = 0;
u64 mutation_inputs = 0;
u8 shall_we_double = 0;

u32 target_inst_ids[INST_SIZE];   // id of target instruction (crash only + top-k)
u32 crash_only_inst_cnt = 0;      // number of crash only instructions

// rewards of predicate-carrying instructions
double cumulative_inst_reward[INST_SIZE][1000];
...
static void select_inst(void){
  double all_reward = 0;
  avg_inst_rewards = ck_realloc(avg_inst_rewards, sizeof(double)*(crash_only_inst_cnt+top_k));
  for (u32 k = 0; k < INST_SIZE; k++) {
    if (target_inst_ids[k] == 0xffffffff)
      break;
    u32 i = target_inst_ids[k];
    u32 tmp_size = inst_selected_cnt[i]<1000? inst_selected_cnt[i]: 1000;
    avg_inst_rewards[k] = 0;
    for(u32 c=0;c<tmp_size;c++)
      avg_inst_rewards[k]+=cumulative_inst_reward[i][c];
    if(tmp_size)
      avg_inst_rewards[k] = avg_inst_rewards[k]/tmp_size;
    all_reward += avg_inst_rewards[k];
  }

  d_inst = ck_realloc(d_inst, sizeof(double)*(crash_only_inst_cnt+top_k));
  for(u32 k=0; k<crash_only_inst_cnt+top_k; k++){
    if(all_reward>0)
      d_inst[k]=avg_inst_rewards[k]/all_reward;
    else
      d_inst[k]=1.000/(crash_only_inst_cnt+top_k);
  }
}

static u32 sample_inst(void){
  d_inst_prime = ck_realloc(d_inst_prime, sizeof(double)*(crash_only_inst_cnt+top_k));
  for(u32 k=0; k<crash_only_inst_cnt+top_k; k++)
    d_inst_prime[k] = (1-seed_selection_gamma)*d_inst[k]+seed_selection_gamma*1.000/(crash_only_inst_cnt+top_k);
...
static void update_inst_state(double reward){
  for(u32 i=0;i<INST_SIZE;i++){
    if (queue_cur && *(queue_cur->trace_bits + MAP_SIZE+INST_SIZE*16 + i) == 0)
      continue;
    cumulative_inst_reward[i][inst_selected_cnt[i]%1000]=reward;
    inst_selected_cnt[i]++;
  }
}
...
static void update_seed_selection_gamma(void) {
  double avg_eward=0;
  if(p_selected_seed){
    u32 tmp_size = inst_selected_cnt[selected_inst_id]<1000? inst_selected_cnt[selected_inst_id]: 1000;
    for(u32 c=0;c<tmp_size;c++)
      avg_eward+=cumulative_inst_reward[selected_inst_id][c];
    avg_eward = avg_eward/tmp_size;
    seed_selection_adaptive+=seed_selection_gamma*avg_eward/((1-seed_selection_gamma)*p_selected_seed+seed_selection_gamma/(crash_only_inst_cnt+top_k));
    seed_selection_gamma = (crash_only_inst_cnt+top_k)/(2*(crash_only_inst_cnt+top_k)+seed_selection_adaptive);
  }
  return;
}

// rewards of mutation locations
double cumulative_loc_reward[2000];
u32 loc_selected_cnt[2000];
u8 mutation_loc_map[2000];
...
static void update_mutation_loc_gamma(double reward) {
  ...
  mutation_loc_adaptive+=mutation_loc_gamma*reward/((1-mutation_loc_gamma)*avg_p_selected_loc+mutation_loc_gamma*(1.000/2000));
  mutation_loc_gamma = 2000*1.000/(2*2000+mutation_loc_adaptive);
  return;
}
...
static inline u8 add_to_global_values(u8 label) {
  if (label) {
    if(global_crash_count==0){
      u32 *count = (u32 *)(trace_bits + MAP_SIZE+INST_SIZE*17);
      for (u32 j = 0; j < INST_SIZE; j++) {
        inst_seq[j] = *(count + 1 + j);
      }
    }
    global_crash_count++;
  } else
  ...
  u64 *layout_info = (u64 *)(trace_bits + MAP_SIZE+INST_SIZE*21+4);
  if(*(layout_info) && *(layout_info)<stack_start)
    stack_start = *(layout_info);
  if(*(layout_info+1)>stack_end)
    stack_end = *(layout_info+1);
  if(*(layout_info+2) && *(layout_info+2)<heap_start)
    heap_start = *(layout_info+2);
  if(*(layout_info+3)>heap_end)
    heap_end = *(layout_info+3);

  u32 i = 0;
  while (i < INST_SIZE) {
    u64 min_v = *((u64 *)(trace_bits + MAP_SIZE + 8 * i));
    u64 max_v = *((u64 *)(trace_bits + MAP_SIZE + INST_SIZE*8 + 8 * i));

    if (*(trace_bits + MAP_SIZE+INST_SIZE*16 + i) == 0) {
      i++;
      continue;
    }
    ...
    u8 found = 0;
    if (global_values[i].min_value == NULL ||
        global_values[i].min_value->value > min_v) {
      temp_v = (struct value_info *)malloc(sizeof(struct value_info));
      temp_v->value = min_v;
      temp_v->crashes_count = 0;
      temp_v->non_crashes_count = 0;
      temp_v->crashes_top_rated = 0;
      temp_v->crashes_was_fuzzed=0;
      temp_v->non_crashes_top_rated = 0;
      temp_v->non_crashes_was_fuzzed=0;

      if (label)
        temp_v->crashes_count++;
      else
        temp_v->non_crashes_count++;

      temp_v->next = global_values[i].min_value;
      global_values[i].min_value = temp_v;
    } else if (global_values[i].min_value->value == min_v) {
      if (label)
        global_values[i].min_value->crashes_count++;
      else
        global_values[i].min_value->non_crashes_count++;
    } else {
      temp_v = global_values[i].min_value;
      while (temp_v->next) {
        if (temp_v->next->value == min_v) {
          if (label)
            temp_v->next->crashes_count++;
          else
            temp_v->next->non_crashes_count++;
          found = 1;
          break;
        }
        if (temp_v->next->value < min_v)
      ...
      if (found == 0) {
        struct value_info *temp_new =
            (struct value_info *)malloc(sizeof(struct value_info));
        temp_new->value = min_v;
        temp_new->crashes_count = 0;
        temp_new->non_crashes_count = 0;
        temp_new->crashes_top_rated = 0;
        temp_new->crashes_was_fuzzed=0;
        temp_new->non_crashes_top_rated = 0;
        temp_new->non_crashes_was_fuzzed=0;

        if (label)
          temp_new->crashes_count++;
        else
          temp_new->non_crashes_count++;

        temp_new->next = temp_v->next;
        temp_v->next = temp_new;
      }
    }

    found = 0;
    if (global_values[i].max_value == NULL ||
        global_values[i].max_value->value > max_v) {
      temp_v = (struct value_info *)malloc(sizeof(struct value_info));
      temp_v->value = max_v;
      temp_v->crashes_count = 0;
      temp_v->non_crashes_count = 0;
      ...
      temp_v->next = global_values[i].max_value;
      global_values[i].max_value = temp_v;
    } else if (global_values[i].max_value->value == max_v) {
      if (label)
        global_values[i].max_value->crashes_count++;
      else
        global_values[i].max_value->non_crashes_count++;
    } else {
      temp_v = global_values[i].max_value;
      while (temp_v->next) {
        if (temp_v->next->value == max_v) {
          if (label)
            temp_v->next->crashes_count++;
          else
            temp_v->next->non_crashes_count++;
...
int converge(){
  u8 converged = 1;
  if(!global_rank_info || global_rank_info->inputs_count<10)
    converged = 0;
...
static void update_predicates() {
  ...
  while (i < INST_SIZE) {
    ...
    double N_global = global_crash_count+global_non_crash_count;
    double H_y_global = -(global_crash_count/N_global*log2(global_crash_count/N_global)+global_non_crash_count/N_global*log2(global_non_crash_count/N_global));
    ...
    while (temp_min) {
      ...
      double ci1 = (t_crash+f_non_crashes)*1.00;
      double cpi1 = t_crash*1.00;
      double cfi1 = f_non_crashes*1.00;
      ...
      double ci2 = (global_values[i].crash_count - t_crash+global_values[i].non_crash_count-f_non_crashes)*1.00;
      double cpi2 = (global_values[i].crash_count - t_crash)*1.00;
      double cfi2 = (global_values[i].non_crash_count-f_non_crashes)*1.00;
      if(cpi2)
        H21  = cpi2/ci2*log2(cpi2/ci2);
      if(cfi2)
        H22 = cfi2/ci2*log2(cfi2/ci2);

      double ci3 = (global_crash_count - global_values[i].crash_count+ global_non_crash_count - global_values[i].non_crash_count)*1.00;
      double cpi3 = (global_crash_count - global_values[i].crash_count)*1.00;
      double cfi3 = (global_non_crash_count - global_values[i].non_crash_count)*1.00;
      if(cpi3)
...
static void rank(){
  ...
  target_inst_ids[crash_only_inst_cnt+top_k] = 0xffffffff;
    
  if (target_inst_ids[crash_only_inst_cnt]!=0xffffffff) {
    ...
    fprintf(ranked_file, "rank update at: %llu, rank_size=%d, crash_only_inst_cnt=%d\n", temp_rank->update_time, temp_rank->rank_size ,crash_only_inst_cnt);
      
    for (u32 j = 0; j<temp_rank->rank_size && target_inst_ids[crash_only_inst_cnt+j]!=0xffffffff; j++) {
      temp_rank->rank_ids[j] = target_inst_ids[crash_only_inst_cnt+j];
      fprintf(ranked_file, "top-%d: %d, score: %lf, predicate: '%c', boundary: %lu\n", j+1,
                target_inst_ids[crash_only_inst_cnt+j], inst_score[target_inst_ids[crash_only_inst_cnt+j]],
                inst_predicate[target_inst_ids[crash_only_inst_cnt+j]], inst_boundary[target_inst_ids[crash_only_inst_cnt+j]]);
    }
...
static void add_to_queue(u8 *fname, u32 len, u8 passed_det, u8 label) {
  ...
  q->trace_bits = ck_alloc(MAP_SIZE+INST_SIZE*21+44);
  memcpy(q->trace_bits, trace_bits, MAP_SIZE+INST_SIZE*21+44);
...
EXP_ST void write_bitmap(void) {
  ...
  if (crashes_bitmap_changed) {
    ...
    ck_write(fd, virgin_bits_crashes, INST_SIZE*64, fname);
  ...
  if (non_crashes_bitmap_changed) {
    ...
    ck_write(fd, virgin_bits_non_crashes, INST_SIZE*64, fname);
...
EXP_ST void read_bitmap(u8 *out_dir) {
  ...
  ck_read(fd, virgin_bits_crashes, INST_SIZE*64, fname);
  ...
  ck_read(fd, virgin_bits_non_crashes, INST_SIZE*64, fname);
...
static inline u8 has_new_crashes_values() {
  ...
  while (i--) {
    if (*(trace_bits + MAP_SIZE + INST_SIZE*16 + i) == 0){ // is_set=0
      continue;
    }
    u64 min_v  = *((u64 *)(trace_bits + MAP_SIZE + 8 * i));
    u64 max_v  = *((u64 *)(trace_bits + MAP_SIZE + INST_SIZE*8 + 8 * i));

    if (inst_predicate[i]=='<' || inst_predicate[i]=='-'){
      u32 min_cksum = hash32(trace_bits + MAP_SIZE + 8 * i, 8, HASH_CONST);
      min_cksum = min_cksum & 0xff;
      if(i==global_last_id || (!is_stack(min_v) && !is_heap(min_v))) {
        u8 *crashes_virgin = virgin_bits_crashes + 64 * i + min_cksum / 8;
        if (unlikely(!((*crashes_virgin) & (1 << (min_cksum % 8))))) {
          if(is_target_inst(i)){
            ...
            if(inst_predicate[i]=='<' && min_v>inst_boundary[i]){
    ...
    if (inst_predicate[i]=='>' || inst_predicate[i]=='-') {
      u32 max_cksum = hash32(trace_bits + MAP_SIZE+INST_SIZE*8 + 8 * i, 8, HASH_CONST);
      max_cksum = max_cksum & 0xff;
      if(i==global_last_id || (!is_stack(max_v) && !is_heap(max_v))) {
        u8* crashes_virgin = virgin_bits_crashes + 64 * i + 32 + max_cksum / 8;
        if (unlikely(!((*crashes_virgin) & (1 << (max_cksum % 8))))) {
          if(is_target_inst(i)){
            ...
            if(inst_predicate[i]=='>' && max_v<=inst_boundary[i]){
              if(new_counterexample_for_predicate[i]==0){
                new_counterexample_for_predicate[i]=1;
                new_counterexample_for_predicate_cnt++;
              }
  ...
  if (unlikely(new_counterexample_for_predicate_cnt)) {
    while (j--) {
      if (*(trace_bits + MAP_SIZE+INST_SIZE*16 + j) == 0) // is_set=0
        continue;

      u64* min_v  = (u64*)(trace_bits + MAP_SIZE + 8 * j);
      u32 min_cksum = hash32(trace_bits + MAP_SIZE + 8 * j, 8, HASH_CONST);
      min_cksum = min_cksum & 0xff;

      u64* max_v  = (u64*)(trace_bits + MAP_SIZE+INST_SIZE*8 + 8 * j);
      u32 max_cksum = hash32(trace_bits + MAP_SIZE+INST_SIZE*8 + 8 * j, 8, HASH_CONST);
      max_cksum = max_cksum & 0xff;

      u8 *crashes_virgin = virgin_bits_crashes + 64 * j + min_cksum / 8;
      *crashes_virgin |= 1 << (min_cksum % 8);

      crashes_virgin = virgin_bits_crashes + 64 * j + 32 + max_cksum / 8;
...
static inline u8 has_new_non_crashes_values() {
  ...
  while (i--) {
    ...
    if (inst_predicate[i]=='<' || inst_predicate[i]=='-'){
      if(i==global_last_id || (!is_stack(min_v) && !is_heap(min_v))) {
        u32 min_cksum = hash32(trace_bits + MAP_SIZE + 8 * i, 8, HASH_CONST);
        min_cksum = min_cksum & 0xff;
        u8 *non_crashes_virgin = virgin_bits_non_crashes + 64 * i + min_cksum / 8;
        if (unlikely(!((*non_crashes_virgin) & (1 << (min_cksum % 8))))) {
          if(is_target_inst(i)){
            ...
            if((inst_predicate[i]=='<' && min_v<=inst_boundary[i]) || (inst_predicate[i]=='-' && min_v>= min_x && min_v<= max_x)){
              if(new_counterexample_for_predicate[i]==0){
                new_counterexample_for_predicate[i]=1;
                new_counterexample_for_predicate_cnt++;
              }
    ...
    if (inst_predicate[i]=='>' || inst_predicate[i]=='-') {
      if(i==global_last_id || (!is_stack(max_v) && !is_heap(max_v))) {
        u32 max_cksum = hash32(trace_bits + MAP_SIZE+INST_SIZE*8 + 8 * i, 8, HASH_CONST);
        max_cksum = max_cksum & 0xff;
        u8 *non_crashes_virgin = virgin_bits_non_crashes + 64 * i + 32 + max_cksum / 8;
        if (unlikely(!((*non_crashes_virgin) & (1 << (max_cksum % 8))))) {
          if(is_target_inst(i)){
            ...
            if((inst_predicate[i]=='>' && max_v>inst_boundary[i]) || (inst_predicate[i]=='-' && max_v>= min_x && max_v<= max_x)){
              if(new_counterexample_for_predicate[i]==0){
                new_counterexample_for_predicate[i]=1;
                new_counterexample_for_predicate_cnt++;
              }
  ...
  if (unlikely(new_counterexample_for_predicate_cnt) || unlikely(new_value_on_crashes_only_inst)) {
    while (j--) {
      if (*(trace_bits + MAP_SIZE+INST_SIZE*16 + j) == 0) // is_set=0
        continue;
      ...
      u8 *non_crashes_virgin = virgin_bits_non_crashes + 64 * j + min_cksum / 8;
      *non_crashes_virgin |= 1 << (min_cksum % 8);

      non_crashes_virgin = virgin_bits_non_crashes + 64 * j + 32 + max_cksum / 8;
  ...
  for (i = 0; i < INST_SIZE; i++) {
    ...
    while (temp_v) {
      if(min_v ==temp_v->value){
        if(q->is_crash){
          if(!temp_v->crashes_top_rated || fav_factor <= temp_v->crashes_top_rated->exec_us * temp_v->crashes_top_rated->len){
            temp_v->crashes_top_rated = q;
            temp_v->crashes_was_fuzzed = 0;
          }
        }else{
          if(!temp_v->non_crashes_top_rated || fav_factor <= temp_v->non_crashes_top_rated->exec_us * temp_v->non_crashes_top_rated->len){
            temp_v->non_crashes_top_rated = q;
            temp_v->non_crashes_was_fuzzed = 0;
          }
        }
        break;
...
EXP_ST void setup_shm(void) {
  ...
  if (!in_bitmap) {
    ...
    memset(virgin_bits_crashes, 0, INST_SIZE*64);
    memset(virgin_bits_non_crashes, 0, INST_SIZE*64);
  }
  ...
  memset(cumulative_loc_reward,0,sizeof(double)*2000);
  memset(loc_selected_cnt, 0, sizeof(u32)*2000);

  memset(cumulative_op_reward,0,sizeof(double)*17);
  memset(op_selected_cnt, 0, sizeof(u32)*17);
  
  memset(cumulative_inst_reward, 0, sizeof(double)*INST_SIZE*1000);
  memset(inst_selected_cnt, 0, sizeof(u32)*INST_SIZE*1000);
  ...
  shm_id = shmget(IPC_PRIVATE, MAP_SIZE+INST_SIZE*21+44, IPC_CREAT | IPC_EXCL | 0600);
...
static u8 run_target(char **argv, u32 timeout) {
  ...
  memset(trace_bits, 0, MAP_SIZE+INST_SIZE*21+44);
...
static u8 calibrate_case(char **argv, struct queue_entry *q, u8 *use_mem,
  ...
  static u8 first_trace[MAP_SIZE+INST_SIZE*21+44];
  ...
  if (q->exec_cksum) {
    ...
    memcpy(first_trace, trace_bits, MAP_SIZE+INST_SIZE*21+44);
  ...
  for (stage_cur = 0; stage_cur < stage_max; stage_cur++) {
    ...
    if (q->exec_cksum != cksum) {
      ...
      if (q->exec_cksum) {
        ...
        q->exec_cksum = cksum;
        q->value_cksum =
            hash32(trace_bits + MAP_SIZE, INST_SIZE*16, HASH_CONST);
        memcpy(first_trace, trace_bits, MAP_SIZE+INST_SIZE*21+44);
        global_last_id = *((u64 *)(trace_bits + MAP_SIZE+INST_SIZE*21+36));
      }
    }
  }
...
static u8 save_if_interesting(char **argv, void *mem, u32 len, u8 fault) {
  ...
  if (fault == crash_mode) {
    ...
    if(global_last_id!=0xffffffffffffffff){
      u64 last_id = *((u64 *)(trace_bits + MAP_SIZE+INST_SIZE*21+36));
    ...
    counterexample_rate = new_counterexample_for_predicate_cnt*1.00/(crash_only_inst_cnt+top_k);
    add_to_global_values(1);
    root_cause_analysis();
  ...
  default:
    ...
    fn = alloc_printf("%s/non_crashes/id_%06u_%u", out_dir, queued_paths,
                      unique_non_crash_id);
...
static u8 trim_case(char **argv, struct queue_entry *q, u8 *in_buf) {
  ...
  static u8 tmp[64];
  static u8 clean_trace[MAP_SIZE+INST_SIZE*21+44];
  ...
  while (remove_len >= MAX(len_p2 / TRIM_END_STEPS, TRIM_MIN_BYTES)) {
    ...
    while (remove_pos < q->len) {
      ...
      cksum = hash32(trace_bits, MAP_SIZE, HASH_CONST);
      value_cksum = hash32(trace_bits + MAP_SIZE, INST_SIZE*16, HASH_CONST);
      ...
      if (cksum == q->exec_cksum && value_cksum == q->value_cksum) {
        ...
        if (!needs_write) {
          ...
          needs_write = 1;
          memcpy(clean_trace, trace_bits, MAP_SIZE+INST_SIZE*21+44);
        }
  ...
  if (needs_write) {
    ...
    memcpy(trace_bits, clean_trace, MAP_SIZE+INST_SIZE*21+44);
    update_value_bitmap_score(q);
  }
...
havoc_stage:
  ...
  for (stage_cur = 0; stage_cur < stage_max; stage_cur++) {
    ...
    memset(mutation_loc_map, 0, 2000);
    p_selected_loc = 0;
    p_selected_loc_cnt = 0;
    u8 loc_greater_than_2000 = 0;
    ...
    if(global_rank_info && ((get_cur_time() - start_time) / 1000-global_rank_info->update_time>1800) && global_rank_info->inputs_count>50){
      stop_soon = 2;
      goto abandon_entry;
    }
    mutation_inputs++;
